# Bodywise Remedy iOS cloud build with GitHub Actions

Bodywise Remedy can now be built on GitHub's macOS runner without owning a Mac.
...
2. Install Bodywise Remedy dependencies.
3. Build the web app.
...
Use the Bundle ID already configured for Bodywise Remedy:
...
This is the most stable no-Mac setup: GitHub imports the certificate/profile into the temporary macOS runner, signs Bodywise Remedy, exports the IPA, then uploads it to App Store Connect/TestFlight.
## How to run the first cloud build

1. Open GitHub.
2. Go to `elaineq81/bodywise`.
3. Open the `Actions` tab.
4. Select `Bodywise Remedy iOS cloud build`.
...
`App Store Connect` → `Bodywise Remedy` → `TestFlight`
...
- The current Bodywise Remedy iOS shell opens the live Bodywise Remedy web app. Before paid App Store launch, add StoreKit/In-App Purchase for premium access.
